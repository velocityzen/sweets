...
Loader.prototype.getSupportedCommands = function() {
	return Loader.super_.prototype.getSupportedCommands().concat(['db_update_scheme', 'db_load_content', 'db_drop', 'create_user']);
};
...
Loader.prototype.run = function () {
	...
	switch (cmd) {
		...
		case 'db_update_scheme':
			this.getWorker().dbDo("update", process.argv[3]);
			break;
		case 'db_drop':
			this.getWorker().dbDo("drop", process.argv[3]);
			break;
		case 'create_user':
			var w = this.getWorker();
			w.createUser.apply(w, process.argv.slice(3));
			break;
		default:
