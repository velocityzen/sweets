...
App.prototype.loadResources = function(localResources, sweetResources) {
	...
	for(var i in sweetResources) {
		...
		resUnit = new UnitSet();
		resUnit.addSet(resModule.namespace, resModule.units.create());
...
App.prototype.dbDo = function (action, resName) {
	this.ensureInited();
	...
	try {
		this.resourceModules.local = require(path.join(process.cwd(), 'lib', 'scheme'));
	} catch(e) {}

	console.log(this.resourceModules.local);
