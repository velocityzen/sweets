...
App.prototype.addUnits = function (options) {
	...
	if(settings.db) {
		var Db = require('./db');
		var DbSweet = this.getSweet(settings.db.sweet).Db;
		units.add('db', new Db(DbSweet));
	}
	...
	if(options.cache) {
		this.loadCacheEngine(options.cache);
	}

	this.loadTemplateEngine(settings.templatePath);
