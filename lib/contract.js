...
var supportedLanguages = new locale.Locales(["en", "ru"]);
...
var viewImpl = function(ctrl) {
	return function(ctx) {
		...
		if (cookies.swts_language) {
			language = cookies.swts_language;
		} else {
			language = new locale.Locales(ctx.req.headers["accept-language"]).best(supportedLanguages).language;
			ctx.res.cookie('swts_language', language, { maxAge: 900000, httpOnly: false });
		}
